...
abstract class Db
{
    ...
    public function autoExecute($table, $data, $type, $where = '', $limit = 0, $use_cache = true, $use_null = false)
    {
        ...
        switch ($type) {
            ...
            default :
                throw new PoGoScannerException('Wrong argument (miss type) in Db::autoExecute()');
        }
    }
    ...
    public function insert($table, $data, $null_values = false, $use_cache = true, $type = Db::INSERT)
    {
        ...
        if ($type == Db::INSERT) {
            ...
            throw new PoGoScannerException('Bad keyword, must be Db::INSERT or Db::INSERT_IGNORE or Db::REPLACE');
        }
            ...
            foreach ($row_data as $key => $value) {
                if (!$first_loop) {
                    ...
                    if (!in_array("`$key`", $keys)) {
                        throw new PoGoScannerException('Keys form $data subarray don\'t match');
                    }

                    if ($duplicate_key_stringified != '') {
                        throw new PoGoScannerException('On duplicate key cannot be used on insert with more than 1 VALUE group');
                    }
                } else {
                    $keys[] = '`'.bqSQL($key).'`';
                }
    ...
    public function executeS($sql, $array = true, $use_cache = true)
    {
        ...
        if (!preg_match('#^\s*\(?\s*(select|show|explain|describe|desc)\s#i', $sql)) {
            if (defined('_MODE_DEV_') && _MODE_DEV_) {
                throw new PoGoScannerException('Db->executeS() must be used only with select, show, explain or describe queries');
            }
    ...
    public function displayError($sql = false)
    {
        ...
        } elseif (_DEBUG_SQL_ && $errno) {
            if ($sql) {
                throw new PoGoScannerException($this->getMsgError().'<br /><br /><pre>'.$sql.'</pre>');
            }

            throw new PoGoScannerException($this->getMsgError());
        }
    }
     ...
     */
    public static function checkCreatePrivilege($server, $user, $pwd, $db, $engine = null)
    {
        return call_user_func_array(array(Db::getClass(), 'checkCreatePrivilege'), array($server, $user, $pwd, $db, $engine));
    }
